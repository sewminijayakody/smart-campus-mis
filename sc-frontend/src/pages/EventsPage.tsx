...
const EventsPage = () => {
  ...
  const [currentMonth, setCurrentMonth] = useState<Date>(new Date("2025-03-01"));
  const [renderKey, setRenderKey] = useState(Date.now());

  // Update state when location changes or on mount
  useEffect(() => {
    const state = location.state as { selectedDate?: Date; registeredEvents?: number[] };
  ...
  const filteredEvents = events.filter((evt) => {
    ...
    console.log("Comparing dates:", eventDate.toDateString(), selectedDate.toDateString());
          ...
          <div className="w-2/3">
            <div className="bg-[#D8EAF3] p-4 rounded-lg shadow-md space-y-4 border border-[#D8EAF3] ml-11 mt-16" key={renderKey}>
              {filteredEvents.length === 0 ? (
                <p className="text-sm text-gray-500">No events available for this date</p>
              ) : (
                filteredEvents.map((evt) => {
                  const isRegistered = registeredEvents.includes(evt.id);
                  console.log(`Rendering event ID: ${evt.id}, Registered: ${isRegistered}`);
                  return (
                    <div
                      key={evt.id}
                    ...
                    >
                      ...
                      <div>
                        ...
                        <p className="text-sm text-gray-500">{new Date(evt.date).toLocaleDateString("en-US", { month: "long", day: "numeric", year: "numeric" })}</p>
